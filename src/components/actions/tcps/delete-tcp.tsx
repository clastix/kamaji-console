...
export const useDeleteTCP = () => {
  const openDialog = useCreateDialog();
  return (tpc: DeleteTCP) => openDialog(<DeleteTCPDialog tcp={tpc} />);
};

const DeleteTCPDialog = ({ tcp }: { tcp: DeleteTCP }) => {
  const router = useRouter();
  const closeDialog = useCloseDialog();
  const utils = reactApi.useContext();
  const deleteMut = reactApi.k8s.deleteTcp.useMutation({
    onSuccess: async () => {
  ...
  return (
    <>
      ...
      >
        Delete Tenant Control Plane {name}
      </Dialog.Title>
      <ZodForm
        schema={CheckSchema}
        onSubmit={async () => {
